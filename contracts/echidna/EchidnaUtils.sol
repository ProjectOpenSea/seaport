...
interface IEchidna {
    function testFulfillAdvancedOrder(bytes32 seed, uint120 numerator, uint120 denominator) external payable;
...
abstract contract EchidnaUtils is IEchidna, EchidnaOrderUtils {
    ...
    event AssertionFailure(string functionName, uint256 params1, uint256 params2);
    ...
    function emitAndFail(string memory message, uint256 num1, uint256 num2) internal {
        emit AssertionFailure(message, num1, num2);
        assert(false);
    }    

    function signOrder(bytes32 orderHash, bytes32 domainSeparator) internal returns(bytes memory) {
    ...
    function mintERC1155(address who, uint256 id, uint256 amount) private {
        _erc1155.mint(who, id, amount);
    }       

    function _assertFundsReceived(address seller, address buyer, uint256 route, uint256 totalTokens, uint256 totalItems, uint uid) internal {
        if (route == 0) /*NATIVE TO ERC721*/ { 
            ...
            if (_erc721.balanceOf(buyer) < totalItems) {
                emitAndFail("/*NATIVE TO ERC721*/ buyer", _erc721.balanceOf(buyer), totalItems);
            }
        } else if (route == 1) /*NATIVE TO ERC1155*/ {
            if(seller.balance < totalTokens) {
                emitAndFail("/*NATIVE TO ERC1155*/ seller", seller.balance, totalTokens);
            }
            if (_erc1155.balanceOf(buyer, uid) < totalTokens) {
                emitAndFail("/*NATIVE TO ERC1155*/ buyer", _erc1155.balanceOf(buyer, uid), totalTokens);
            }
        } else if (route == 2) /*ERC20 TO ERC721*/ { 
            if (_erc20.balanceOf(seller) < totalTokens) {
                emitAndFail("/*ERC721 TO ERC20 */ FAILED", _erc20.balanceOf(seller), totalTokens);
            }
            if (_erc721.balanceOf(buyer) < totalItems) {
                emitAndFail("/*ERC721 TO ERC20 */ FAILED", _erc721.balanceOf(buyer), totalItems);
            }
        } else if (route == 3) /*ERC20 TO ERC1155*/ { 
            if (_erc1155.balanceOf(buyer, uid) < totalTokens) {
                emitAndFail(" /*ERC115 TO ERC20 */ buyer", _erc1155.balanceOf(buyer, uid), totalTokens);
            }
            if (_erc20.balanceOf(seller) < totalTokens) {
                emitAndFail(" /*ERC115 TO ERC20 */ seller", _erc20.balanceOf(seller), totalTokens);
            }
        } else if (route == 4) /*ERC721 TO ERC20 */ {     
            if (_erc20.balanceOf(buyer) < totalTokens) {
                emitAndFail("/*ERC20 TO ERC721*/ FAILED", _erc20.balanceOf(buyer), totalTokens);
            }
            if (_erc721.balanceOf(seller) < totalItems) {
                emitAndFail("/*ERC20 TO ERC721*/ FAILED", _erc721.balanceOf(seller), totalItems);
            }
        } else if (route == 5) /*ERC115 TO ERC20 */ { 
            if (_erc1155.balanceOf(seller, uid) < totalTokens) {
                emitAndFail(" /*ERC20 TO ERC1155*/ seller", _erc1155.balanceOf(seller, uid), totalTokens);
            }
            if (_erc20.balanceOf(buyer) < totalTokens) {
                emitAndFail(" /*ERC20 TO ERC1155*/ buyer", _erc20.balanceOf(buyer), totalTokens);
            }
        }
    }          

    function createOrderParameters(address payable seller, address payable buyer, bytes32 seed, uint route, bool isBasic) internal returns (OrderParameters memory orderParams, uint totalTokens, uint totalItems, uint uid) {
        uint amount = uint256(uint112(uint256(seed)));
        // Prevent MissingItemAmount revert
        require(amount != 0);
        // Unique id for ERC721 and ERC1155 ids
        uid = uint(keccak256(abi.encode(seed)));
        ...
        totalItems = isBasic ? 1 : uint(seed) % 10 + 1; 
        OfferItem[] memory offer = new OfferItem[](totalItems);  
        ConsiderationItem[] memory consideration = new ConsiderationItem[](totalItems);        
        ...
        if (route == 0) /*ETH_TO_ERC721*/ { 
            ...
            for (uint i = 0; i < totalItems; i++) {
                offer[i] = createOfferItem(ItemType.ERC721, address(_erc721), uid, 1, 1);
                require(_erc721.ownerOf(uid) == address(0));
                mintERC721(seller, uid);
                consideration[i] = createConsiderationItem(ItemType.NATIVE, address(0), uint256(0), amount, amount, seller);
                uid++;
            }
            ...
            for (uint i = 0; i < totalItems; i++) {
                offer[i] = createOfferItem(ItemType.ERC1155, address(_erc1155), uid, amount, amount);
                consideration[i] = createConsiderationItem(ItemType.NATIVE, address(0), uint256(0), amount, amount, seller);
            }
            ...
            for (uint i = 0; i < totalItems; i++) {
                ...
                mintERC721(seller, uid);     
                offer[i] = createOfferItem(ItemType.ERC721, address(_erc721), uid, 1, 1);          
                consideration[i] = createConsiderationItem(ItemType.ERC20,address(_erc20),0, amount, amount, seller);
                uid++;
            }
            ...
            for (uint i = 0; i < totalItems; i++) {
                ...
                offer[i] = createOfferItem(ItemType.ERC20, address(_erc20), uint256(0), amount, amount);
                consideration[i] = createConsiderationItem(ItemType.ERC721, address(_erc721), uid, 1, 1, seller);
                uid++;
            }
            ...
            for (uint i = 0; i < totalItems; i++) {
                offer[i] = createOfferItem(ItemType.ERC20, address(_erc20), uint256(0), amount, amount);
                consideration[i] = createConsiderationItem(ItemType.ERC1155, address(_erc1155), uid, amount, amount, seller);
            }
        }
        orderParams =
            OrderParameters({
                ...
                orderType: OrderType(uint(seed) % 2), 
