...
contract SignedZone is SignedZoneEventsAndErrors, ZoneInterface, SIP5Interface {
    ...
    uint256 private constant _NEW_ORDER = 0;
    uint256 private constant _FULFILLED_OR_CANCELLED = 1;
    mapping(bytes32 => uint256) public remainingSpendLimit;
                ...
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encode(
                            ...
                            signedOrder.fulfiller,
                            signedOrder.expiration,
                            signedOrder.orderHash,
                            signedOrder.considerationHash
                        )
                    )
                )
            );
    ...
    event SignedZoneValidateOrderEvent(ZoneParameters zoneParameters);
    event SignedZoneContextEvent(
        address fulfiller,
        uint64 expiration,
        bytes32 orderHash,
        bytes context
    );
    event MyTestEvent(uint256 spent);
    ...
    function validateOrder(
        ...
        emit SignedZoneValidateOrderEvent(zoneParameters);

        ProOrderExtraData memory extraData = abi.decode(
            zoneParameters.extraData,
            (ProOrderExtraData)
        );

        bytes32 signedOrderHash = hashSignedOrder(
            SignedOrder(
                extraData.fulfiller,
                extraData.expiration,
                extraData.orderHash,
                extraData.considerationHash
            )
        );

        // STEP A1: Verify server signature
        // We'll verify (fulfiller, orderHash, expiration, hash of ReceivedItem array)
        // by hashing and recovering the signature signer.
        require(
            _signers[ECDSA.recover(signedOrderHash, extraData.signature)],
            "validateOrder: invalid server signature"
        );

        // STEP A2: Verify fulfiller
        // We'll verify the fulfiller by comparing it with the
        // fulfiller from the zoneParameters.
        require(
            extraData.fulfiller == zoneParameters.fulfiller,
            "validateOrder: invalid fulfiller"
        );

        // STEP A3: Verify orderHash
        // We'll verify the orderHash by checking if orderHashes[] from zoneParameters
        // includes orderHash
        bool matchingOrderHash = false;
        for (uint256 i = 0; i < zoneParameters.orderHashes.length; i++) {
            if (zoneParameters.orderHashes[i] == extraData.orderHash) {
                matchingOrderHash = true;
                break;
            }
        }
        require(matchingOrderHash, "validateOrder: invalid orderHash");

        // STEP A4: Verify signature expiration
        // We'll check if the current timestamp and compare it with signature expiration
        // timestamp.
        require(
            extraData.expiration >= uint64(block.timestamp),
            "validateOrder: expired signature"
        );

        // STEP A5: Verify consideration items
        ...
        require(
            keccak256(abi.encode(zoneParameters.consideration)) ==
                extraData.considerationHash,
            "validateOrder: incorrect consideration items"
        );

        // STEP B1: Verify minEthPricePerItem, maxEthPricePerItem, ethSpendLimit
        // As we know that the zoneHash is valid, we will hash
        // (minEthPricePerItem, maxEthPricePerItem, ethSpendLimit) to check if that
        // matches the zoneHash.
        require(
            keccak256(
                abi.encode(
                    extraData.minEthPricePerItem,
                    extraData.maxEthPricePerItem,
                    extraData.ethSpendLimit
                )
            ) == zoneParameters.zoneHash,
            "validateOrder: incorrect extraData parameters"
        );

        // STEP B2: Verify if per item price is within
        // (minEthPricePerItem, maxEthPricePerItem) range
        ...
        for (uint256 i = 0; i < zoneParameters.offer.length; i++) {
            if (
                ...
                totalSpent = totalSpent + zoneParameters.offer[i].amount;
            }
        }
        ...
        for (uint256 i = 0; i < zoneParameters.consideration.length; i++) {
            if (
                ...
                itemBuyCount = itemBuyCount + zoneParameters.offer[i].amount;
            }
        }

        uint256 spent = totalSpent / itemBuyCount;

        require(
            spent >= extraData.minEthPricePerItem &&
                spent < extraData.maxEthPricePerItem,
            "validateOrder: incorrect pricePerItem"
        );

        // STEP B3: Verify if we breached ethSpendLimit
        // If extraData specifies that this order uses ethSpendLimit, then update
        // remainingSpendLimit and make sure that we do not breach the spend limit.
        if (extraData.ethSpendLimit > 0) {
            // In case it was a new order
            // NOTE: We assume that the trx will use maximum gas allowed by the user
            if (remainingSpendLimit[extraData.orderHash] == _NEW_ORDER) {
                remainingSpendLimit[extraData.orderHash] =
                    _FULFILLED_OR_CANCELLED +
                    extraData.ethSpendLimit -
                    totalSpent;
            }
            // In case it was a unfilled order
            // NOTE: We assume that the trx will use maximum gas allowed by the user
            else {
                remainingSpendLimit[extraData.orderHash] =
                    remainingSpendLimit[extraData.orderHash] -
            ...
            // Check to make sure we didn't buy more than specified in the order
            require(
                remainingSpendLimit[extraData.orderHash] >=
                    _FULFILLED_OR_CANCELLED,
                "validateOrder: ethSpendLimit exceeded"
            );
        }
