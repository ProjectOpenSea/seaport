...
import {AmountGenerator, Amount, AmountDegree} from "../../../contracts/helpers/sol/lib/generators/AmountGenerator.sol";
import "seaport-sol/SeaportSol.sol";

contract GeneratorExamplesTest is BaseOrderTest {
    ...
    function testFulfillAscendingDescendingOffer(
        ...
        fuzzParams.high = uint120(
            bound(fuzzParams.high, 1, type(uint120).max)
        );
        fuzzParams.low = uint120(
            bound(fuzzParams.low, 1, type(uint120).max)
        );
        vm.assume(fuzzParams.high > fuzzParams.low);
        fuzzParams.warpAmount = uint16(bound(fuzzParams.warpAmount, 0, 1000));

        fuzzParams.amount = uint8(bound(fuzzParams.amount, 1, 2));
    ...
    function setUpFulfillAscendingDescendingOffer(
        ...
        // New scope to set up order/consideration
        {
            ...
            // Use generator to set up ascending/descending amounts
            offers[0] = AmountGenerator.generate(
                offerItem,
                Amount(context.fuzzParams.amount),
                AmountDegree.MEDIUM,
                context.fuzzParams.high,
                context.fuzzParams.low
            );
    ...
    function execFulfillAscendingDescendingOffer(
        ...
        Amount amount = Amount(context.fuzzParams.amount);
        uint256 expectedAmount = _locateCurrentAmount(
            (amount == Amount.ASCENDING ? context.fuzzParams.low : context.fuzzParams.high),
            (amount == Amount.ASCENDING ? context.fuzzParams.high : context.fuzzParams.low),
            startTime,
            endTime,
            false // don't round up offers
        );

        vm.expectEmit(true, true, true, true, address(erc20s[0]));
    ...
    function setUpFulfillAscendingDescendingConsideration(
        ...
        // New scope to setup order/consideration
        {
            ...
            erc1155s[0].mint(offerer1.addr, 1, 1000);

            // Offer an ERC1155 with fixed amount
            ...
            considerations[0] = ConsiderationItemLib
                ...
                .withItemType(ItemType.ERC20)
                .withToken(address(erc20s[0]))
                .withStartAmount(context.fuzzParams.high)
                .withEndAmount(context.fuzzParams.low);
        }
