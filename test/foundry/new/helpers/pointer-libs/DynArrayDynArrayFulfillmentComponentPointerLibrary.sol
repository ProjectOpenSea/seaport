...
library DynArrayDynArrayFulfillmentComponentPointerLibrary {
  enum ScuffKind { LengthOverflow, element_HeadOverflow, element_LengthOverflow }

  uint256 internal constant CalldataStride = 0x20;
  uint256 internal constant MinimumElementScuffKind = uint256(ScuffKind.element_LengthOverflow);
  uint256 internal constant MaximumElementScuffKind = uint256(ScuffKind.element_LengthOverflow);
  ...
  function addDirtyBitsToLength(DynArrayDynArrayFulfillmentComponentPointer ptr) internal pure {
  ...
  function elementData(DynArrayDynArrayFulfillmentComponentPointer ptr, uint256 index) internal pure returns (DynArrayFulfillmentComponentPointer) {
    return DynArrayFulfillmentComponentPointerLibrary.wrap(ptr.unwrap().offset(elementHead(ptr, index).readUint256()));
  }
  ...
  function addScuffDirectives(DynArrayDynArrayFulfillmentComponentPointer ptr, ScuffDirectivesArray directives, uint256 kindOffset) internal pure {
