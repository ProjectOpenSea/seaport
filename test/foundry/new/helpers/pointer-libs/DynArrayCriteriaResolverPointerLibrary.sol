...
library DynArrayCriteriaResolverPointerLibrary {
  enum ScuffKind { LengthOverflow, element_HeadOverflow, element_side_Overflow, element_criteriaProof_HeadOverflow, element_criteriaProof_LengthOverflow }

  uint256 internal constant CalldataStride = 0x20;
  uint256 internal constant MinimumElementScuffKind = uint256(ScuffKind.element_side_Overflow);
  uint256 internal constant MaximumElementScuffKind = uint256(ScuffKind.element_criteriaProof_LengthOverflow);
  ...
  /// @dev Add dirty bits from 0 to 224 to the length for the `CriteriaResolver[]` at `ptr`
  function addDirtyBitsToLength(DynArrayCriteriaResolverPointer ptr) internal pure {
  ...
  function elementData(DynArrayCriteriaResolverPointer ptr, uint256 index) internal pure returns (CriteriaResolverPointer) {
    return CriteriaResolverPointerLibrary.wrap(ptr.unwrap().offset(elementHead(ptr, index).readUint256()));
  }
  ...
  function addScuffDirectives(DynArrayCriteriaResolverPointer ptr, ScuffDirectivesArray directives, uint256 kindOffset) internal pure {
    /// @dev Overflow length of CriteriaResolver[]
  ...
  function toString(ScuffKind k) internal pure returns (string memory) {
    ...
    if (k == ScuffKind.element_side_Overflow) return "element_side_Overflow";
    if (k == ScuffKind.element_criteriaProof_HeadOverflow) return "element_criteriaProof_HeadOverflow";
    return "element_criteriaProof_LengthOverflow";
  }
