...
library GetOrderHashPointerLibrary {
  enum ScuffKind { order_offerer_DirtyBits, order_zone_DirtyBits, order_offer_element_itemType_DirtyBits, order_offer_element_itemType_MaxValue, order_offer_element_token_DirtyBits, order_consideration_element_itemType_DirtyBits, order_consideration_element_itemType_MaxValue, order_consideration_element_token_DirtyBits, order_consideration_element_recipient_DirtyBits, order_orderType_DirtyBits, order_orderType_MaxValue }
  ...
  function encodeFunctionCall(OrderComponents memory _order) internal pure returns (bytes memory) {
    return abi.encodeWithSignature("getOrderHash((address,address,(uint8,address,uint256,uint256,uint256)[],(uint8,address,uint256,uint256,uint256,address)[],uint8,uint256,uint256,bytes32,uint256,bytes32,uint256))", _order);
  }

  /// @dev Encode function call from arguments
  function fromArgs(OrderComponents memory _order) internal pure returns (GetOrderHashPointer ptrOut) {
    bytes memory data = encodeFunctionCall(_order);
