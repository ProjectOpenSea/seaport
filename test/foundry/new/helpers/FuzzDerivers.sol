...
library FuzzDerivers {
     ...
     */
    function withDerivedCallValue(FuzzTestContext memory context)
        internal
     ...
     * @param context A Fuzz test context.
     */
    function withDerivedCriteriaResolvers(FuzzTestContext memory context)
        internal
        view
        returns (FuzzTestContext memory)
    {
     ...
     * @param context A Fuzz test context.
     */
    function withDerivedOrderDetails(FuzzTestContext memory context)
        internal
        view
        returns (FuzzTestContext memory)
    {
        ...
        for (uint256 i; i < context.advancedOrdersSpace.orders.length; ++i) {
            unavailableReasons[i] =
                context.advancedOrdersSpace.orders[i].unavailableReason;
        }
        ...
        for (uint256 i; i < context.executionState.orders.length; ++i) {
            ...
            if (
                ...
                if (!(block.timestamp < order.endTime)) {
                    ...
                    context.executionState.orderDetails[i].unavailableReason =
                        UnavailableReason.STARTS_IN_FUTURE;
                } else if (
     ...
     * @param context A Fuzz test context.
     */
    function withDerivedFulfillments(FuzzTestContext memory context)
        internal
        pure
        returns (FuzzTestContext memory)
    {
        ...
        (
            ...
            Fulfillment[] memory fulfillments,
            MatchComponent[] memory remainingOfferComponents
        ) = getDerivedFulfillments(context, context.executionState.orderDetails);
        ...
        context.executionState.remainingOfferComponents =
            remainingOfferComponents.toFulfillmentComponents();
        ...
        )
    {
        ...
        if (
            ...
            (implicitExecutionsPost, nativeTokensReturned) = context
                .toFulfillmentDetails(nativeTokensSupplied).getStandardExecutions();
        } else if (
            ...
            // functions.
            (implicitExecutionsPost, nativeTokensReturned) = context
                .toFulfillmentDetails(nativeTokensSupplied).getBasicExecutions();
        } else if (
            ...
            ) = context.toFulfillmentDetails(nativeTokensSupplied)
     ...
     * @param context A Fuzz test context.
     */
    function withDerivedExecutions(FuzzTestContext memory context)
        internal
        returns (FuzzTestContext memory)
    {
        (
        ...
        if (
            ...
            context.expectations.expectedImpliedNativeExecutions =
                expectedImpliedNativeExecutions - nativeTokensReturned;
        }
