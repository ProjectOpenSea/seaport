...
contract SeaportValidatorTestSuite is BaseOrderTest, SeaportValidatorTest {
    ...
    function setUp() public override(BaseOrderTest, SeaportValidatorTest) {
        ...
        offer[0] = OfferItemLib.empty().withItemType(ItemType.ERC20).withToken(
            address(erc20s[0])
        ).withAmount(1);
        ...
        parameters = OrderComponentsLib.fromDefault(STANDARD).toOrderParameters(
        ).withOffer(offer);
        ...
        offer = new OfferItem[](1);
        offer[0] = OfferItemLib.empty().withItemType(ItemType.ERC1155).withToken(
            address(erc1155s[0])
        ).withIdentifierOrCriteria(1).withAmount(1);
        parameters = OrderComponentsLib.fromDefault(STANDARD).toOrderParameters(
        ).withOffer(offer);
        ...
        offer = new OfferItem[](1);
        offer[0] = OfferItemLib.empty().withItemType(ItemType.NATIVE).withToken(
            address(0)
        ).withIdentifierOrCriteria(0).withAmount(1);
        parameters = OrderComponentsLib.fromDefault(STANDARD).toOrderParameters(
        ).withOffer(offer);
        ...
        _consideration[0] = ConsiderationItemLib.empty().withItemType(
            ItemType.NATIVE
        ).withToken(address(0)).withAmount(1);
    ...
    function test_empty_isValidOrder() public {
        ErrorsAndWarnings memory actual =
            validator.isValidOrder(OrderLib.empty(), address(seaport));

        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(TimeIssue.EndTimeBeforeStartTime).addError(
            SignatureIssue.Invalid
        ).addError(GenericIssue.InvalidOrderFormat).addWarning(
    ...
    function test_default_full_isValidOrder() public {
        ErrorsAndWarnings memory actual = validator.isValidOrder(
            OrderLib.fromDefault(STANDARD), address(seaport)
        );

        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(SignatureIssue.Invalid).addError(
            GenericIssue.InvalidOrderFormat
        ).addWarning(TimeIssue.ShortOrder).addWarning(OfferIssue.ZeroItems)
            .addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_erc20_identifierNonZero() public {
        ...
        ErrorsAndWarnings memory actual =
            validator.isValidOrder(order, address(seaport));

        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(ERC20Issue.IdentifierNonZero).addError(SignatureIssue.Invalid)
            .addError(GenericIssue.InvalidOrderFormat).addWarning(
            TimeIssue.ShortOrder
        ).addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_erc20_invalidToken() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(ERC20Issue.InvalidToken).addError(SignatureIssue.Invalid)
            .addError(GenericIssue.InvalidOrderFormat).addWarning(
            TimeIssue.ShortOrder
        ).addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_erc20_insufficientAllowance() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(ERC20Issue.InsufficientAllowance).addError(
            SignatureIssue.Invalid
        ).addError(GenericIssue.InvalidOrderFormat).addWarning(
            TimeIssue.ShortOrder
        ).addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_erc20_insufficientBalance() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            ...
            GenericIssue.InvalidOrderFormat
        ).addWarning(TimeIssue.ShortOrder).addWarning(
            ConsiderationIssue.ZeroItems
        );
    ...
    function test_isValidOrder_erc721_amountNotOne() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(ERC721Issue.AmountNotOne).addError(SignatureIssue.Invalid)
            .addError(GenericIssue.InvalidOrderFormat).addWarning(
            TimeIssue.ShortOrder
        ).addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_erc721_invalidToken() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(ERC721Issue.InvalidToken).addError(SignatureIssue.Invalid)
            .addError(GenericIssue.InvalidOrderFormat).addWarning(
            TimeIssue.ShortOrder
        ).addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_erc721_identifierDNE() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(ERC721Issue.NotOwner).addError(ERC721Issue.NotApproved)
            .addError(ERC721Issue.IdentifierDNE).addError(SignatureIssue.Invalid)
            .addError(GenericIssue.InvalidOrderFormat).addWarning(
            TimeIssue.ShortOrder
    ...
    function test_isValidOrder_erc721_notApproved() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(ERC721Issue.NotApproved).addError(SignatureIssue.Invalid)
            .addError(GenericIssue.InvalidOrderFormat).addWarning(
            TimeIssue.ShortOrder
        ).addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_erc721_criteriaNotPartialFill() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(OfferIssue.AmountVelocityHigh).addError(
    ...
    function test_isValidOrder_erc1155_invalidToken() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(ERC1155Issue.InvalidToken).addError(SignatureIssue.Invalid)
            .addError(GenericIssue.InvalidOrderFormat).addWarning(
            TimeIssue.ShortOrder
        ).addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_erc1155_notApproved() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(ERC1155Issue.NotApproved).addError(SignatureIssue.Invalid)
            .addError(GenericIssue.InvalidOrderFormat).addWarning(
            TimeIssue.ShortOrder
        ).addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_erc1155_insufficientBalance() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(ERC1155Issue.NotApproved).addError(
            ERC1155Issue.InsufficientBalance
        ).addError(SignatureIssue.Invalid).addError(
            GenericIssue.InvalidOrderFormat
        ).addWarning(TimeIssue.ShortOrder).addWarning(
            ConsiderationIssue.ZeroItems
    ...
    function test_isValidOrder_statusIssue_cancelled() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(StatusIssue.Cancelled).addError(ERC721Issue.NotOwner).addError(
            ERC721Issue.NotApproved
        ).addError(SignatureIssue.Invalid).addError(
            GenericIssue.InvalidOrderFormat
        ).addWarning(TimeIssue.ShortOrder).addWarning(
            ConsiderationIssue.ZeroItems
    ...
    function test_isValidOrder_statusIssue_contractOrder() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            ...
            .addError(GenericIssue.InvalidOrderFormat).addWarning(
            TimeIssue.ShortOrder
        ).addWarning(StatusIssue.ContractOrder).addWarning(
            ConsiderationIssue.ZeroItems
    ...
    function test_isValidOrder_timeIssue_expired() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(TimeIssue.Expired).addError(ERC721Issue.NotOwner).addError(
            ERC721Issue.NotApproved
        ).addError(SignatureIssue.Invalid).addError(
            GenericIssue.InvalidOrderFormat
        ).addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_timeIssue_distantExpiration() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            ...
            .addError(SignatureIssue.Invalid).addError(
            GenericIssue.InvalidOrderFormat
        ).addWarning(TimeIssue.DistantExpiration).addWarning(
            ConsiderationIssue.ZeroItems
        );
    ...
    function test_isValidOrder_timeIssue_notActive_shortOrder() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            ...
            .addError(SignatureIssue.Invalid).addError(
            GenericIssue.InvalidOrderFormat
        ).addWarning(TimeIssue.NotActive).addWarning(TimeIssue.ShortOrder)
            .addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_conduitIssue_keyInvalid() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(ConduitIssue.KeyInvalid).addError(SignatureIssue.Invalid)
            .addError(GenericIssue.InvalidOrderFormat).addWarning(
            TimeIssue.ShortOrder
        ).addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_offerIssue_moreThanOneItem() public {
        ...
        offer[1] = OfferItemLib.empty().withItemType(ItemType.ERC721).withToken(
            address(erc721s[0])
    ...
    function test_isValidOrder_offerIssue_amountZero() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(OfferIssue.AmountZero).addError(SignatureIssue.Invalid)
            .addError(GenericIssue.InvalidOrderFormat).addWarning(
            TimeIssue.ShortOrder
        ).addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_offerIssue_nativeItem() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(SignatureIssue.Invalid).addError(
            GenericIssue.InvalidOrderFormat
        ).addWarning(TimeIssue.ShortOrder).addWarning(OfferIssue.NativeItem)
            .addWarning(ConsiderationIssue.ZeroItems);
    ...
    function test_isValidOrder_offerIssue_duplicateItem() public {
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(OfferIssue.DuplicateItem).addError(SignatureIssue.Invalid)
            .addError(GenericIssue.InvalidOrderFormat).addWarning(
            TimeIssue.ShortOrder
        ).addWarning(OfferIssue.MoreThanOneItem).addWarning(
            ConsiderationIssue.ZeroItems
        );
    ...
    function test_isValidOrder_offerIssue_amountVelocityHigh() public {
        ...
        offer[0] = OfferItemLib.empty().withItemType(ItemType.ERC20).withToken(
            address(erc20s[0])
        ).withIdentifierOrCriteria(0).withStartAmount(1e16).withEndAmount(1e25);
    ...
    function test_isValidOrder_offerIssue_amountStepLarge() public {
        ...
        offer[0] = OfferItemLib.empty().withItemType(ItemType.ERC20).withToken(
            address(erc20s[0])
        ).withIdentifierOrCriteria(0).withStartAmount(1e10).withEndAmount(1e11);
        ...
        ErrorsAndWarnings memory expected = ErrorsAndWarningsLib.empty()
            .addError(SignatureIssue.Invalid).addError(
            GenericIssue.InvalidOrderFormat
        ).addWarning(OfferIssue.AmountStepLarge).addWarning(
            ConsiderationIssue.ZeroItems
        );
