...
import {ERC20Interface, ERC721Interface} from "seaport/interfaces/AbridgedTokenInterfaces.sol";
...
import {SpentItem, ReceivedItem, InventoryUpdate} from "seaport/lib/ConsiderationStructs.sol";
...
import {TestPoolOfferer} from "seaport/test/TestPoolOfferer.sol";
import {TestPoolFactory} from "seaport/test/TestPoolFactory.sol";
...
contract TestPoolFactoryImpl {
    ...
    function createPoolOfferer(address erc721, uint256[] calldata tokenIds, address erc20, uint256 amount)
        external
        returns (address newPool)
    {
        newPool = address(new TestPoolImpl(seaport, erc721, tokenIds, erc20, amount));
...
contract TestPoolOffererTest is Test {
    ...
    function setUp() public {
        seaport = address(new Seaport(address(new ConduitController())));
        erc20 = new TestERC20();
        ...
        test = TestPoolImpl(factory.createPoolOfferer(address(erc721), tokenIds, address(erc20), 1e18));
    }
    ...
    function testPreviewOrder() public {
        ...
        minimumReceived[0] = SpentItem({itemType: ItemType.ERC721, token: address(erc721), identifier: 0, amount: 1});
        ...
        assertEq(uint8(spentItems[0].itemType), uint8(ItemType.ERC721), "wrong spentitem type");
        ...
        assertEq(uint8(receivedItems[0].itemType), uint8(ItemType.ERC20), "wrong receiveditem type");
    ...
    function testPreviewOrder2() public {
        ...
        minimumReceived[0] = SpentItem({itemType: ItemType.ERC721, token: address(erc721), identifier: 0, amount: 1});
        minimumReceived[1] = SpentItem({itemType: ItemType.ERC721, token: address(erc721), identifier: 1, amount: 1});
        SpentItem[] memory maximumSpent = new SpentItem[](1);
        maximumSpent[0] = SpentItem(ItemType.ERC20, address(erc20), 6e17, 1);
        (SpentItem[] memory spentItems, ReceivedItem[] memory receivedItems) =
            test.previewOrder(address(0), minimumReceived, maximumSpent, "");
        ...
        assertEq(spentItems[0].identifier, 0, "wrong spentitem identifier");
        assertEq(spentItems[0].token, address(erc721), "wrong spentitem token");
        assertEq(uint8(spentItems[1].itemType), uint8(ItemType.ERC721), "wrong spentitem type");
    ...
    function testPreviewOrder3() public {
        ...
        maximumSpent[0] = SpentItem({itemType: ItemType.ERC721, token: address(erc721), identifier: 3, amount: 1});
        maximumSpent[1] = SpentItem({itemType: ItemType.ERC721, token: address(erc721), identifier: 4, amount: 1});
        (SpentItem[] memory spentItems, ReceivedItem[] memory receivedItems) =
            test.previewOrder(address(0), minimumReceived, maximumSpent, "");

        assertEq(spentItems.length, 1, "wrong spentItems length");
        assertEq(uint8(spentItems[0].itemType), uint8(ItemType.ERC20), "wrong receiveditem type");
        ...
        assertEq(uint8(receivedItems[0].itemType), uint8(ItemType.ERC721), "wrong spentitem type");
        assertEq(receivedItems[0].amount, 1, "wrong spentitem amount");
        assertEq(receivedItems[0].identifier, 3, "wrong spentitem identifier");
        assertEq(receivedItems[0].token, address(erc721), "wrong spentitem token");
        assertEq(receivedItems[0].recipient, address(test), "wrong receiver");

        assertEq(uint8(receivedItems[1].itemType), uint8(ItemType.ERC721), "wrong spentitem type");
        assertEq(receivedItems[1].amount, 1, "wrong spentitem amount");
        assertEq(receivedItems[1].identifier, 4, "wrong spentitem identifier");
        assertEq(receivedItems[1].token, address(erc721), "wrong spentitem token");
    ...
    function testGenerateOrder() public {
        ...
        assertEq(test.getInternalBalance(), 1e18-4e17, "wrong internal balance");
        assertEq(test.getInternalTokenBalance(), 5, 'wrong internal token balance');
    ...
    function testGenerateOrder2() public {
        ...
        assertEq(test.getInternalBalance(), 1e18+2e18, "wrong internal balance");
        assertEq(test.getInternalTokenBalance(), 1, 'wrong internal token balance');
        assertFalse(test.inTokenIds(0), 'id not in tokenIds');
        assertFalse(test.inTokenIds(1), 'id not in tokenIds');
    }
}
