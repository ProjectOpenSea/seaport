...
describe.skip(`Zone - SignedZone (Seaport v${VERSION})`, function () {
  ...
  const salt = `0x${deployConstants.SIGNED_ZONE_CAPTAIN_ADDRESS.slice(
    2
  )}0f98040e5acf2000012416d4`;
  ...
  beforeEach(async () => {
    ...
    await provider.send("hardhat_impersonateAccount", [
      "0x86D26897267711ea4b173C8C124a0A73612001da",
    ]);
    await faucet("0x86D26897267711ea4b173C8C124a0A73612001da", provider);
    ...
    await create2Factory.findCreate2Address(
      deployConstants.SIGNED_ZONE_CONTROLLER_CREATION_SALT,
    ...
    await create2Factory.findCreate2Address(
      deployConstants.SIGNED_ZONE_CAPTAIN_CREATION_SALT,
      SignedZoneCaptainFactory.bytecode +
        deployConstants.SIGNED_ZONE_CONTROLLER_ADDRESS.slice(2).padStart(
          64,
          "0"
        )
    );
    ...
    const signedZoneDeployerFactory = await ethers.getContractFactory(
      "SignedZoneDeployer",
      deployer
    );

    await signedZoneDeployerFactory
      .connect(deployer)
      .deploy(
        owner.address,
        rotator.address,
    ...
    signedZoneController = (await ethers.getContractAt(
      "SignedZoneController",
      deployConstants.SIGNED_ZONE_CONTROLLER_ADDRESS,
      owner
    ...
    signedZoneCaptain = (await ethers.getContractAt(
      "SignedZoneCaptain",
      deployConstants.SIGNED_ZONE_CAPTAIN_ADDRESS,
      owner
    )) as SignedZoneCaptain;
    ...
    expect(signedZone.address).to.equal(deployConstants.SIGNED_ZONE_ADDRESS);
  ...
  it.skip("Fulfills an order with a signed zone", async () => {
  ...
  it.skip("Fulfills an order with a signed zone for a specific fulfiller only", async () => {
    // Execute 721 <=> ETH order
    const nftId = await mintAndApprove721(seller, marketplaceContract.address);
  ...
  it.skip("Fulfills an advanced order with criteria with a signed zone", async () => {
  ...
  it.skip("Does not fulfill an expired signature order with a signed zone", async () => {
    // Create advanced order using signed zone
    // Execute 721 <=> ETH order
    const nftId = await mintAndApprove721(seller, marketplaceContract.address);
  ...
  it.skip("Transfer ownership of zones via a two-stage process", async () => {
    await expect(
      signedZoneCaptain
        .connect(buyer)
  ...
  it.skip("Transfer ownership of the captain via a two-stage process", async () => {
    await expect(
  ...
  it.skip("Reverts if the signedzone is sent unsupported function selector", async () => {
  ...
  it.skip("Reverts if ZoneParameters has non-default offset", async () => {
    const invalidOffset =
  ...
  it.skip("Revert: Fulfills an order without the correct substandard version", async () => {
    // Execute 721 <=> ETH order
    const nftId = await mintAndApprove721(seller, marketplaceContract.address);
  ...
  it.skip("Revert: Try to fulfill an order with an incorrect token identifier", async () => {
    // Execute 721 <=> ETH order
    const nftId = await mintAndApprove721(seller, marketplaceContract.address);
  ...
  it.skip("Revert: Try to fulfill an order with no considerations", async () => {
    // Execute 721 <=> ETH order
    const nftId = await mintAndApprove721(seller, marketplaceContract.address);
  ...
  it.skip("Update the rotator of the signed zone captain", async () => {
  ...
  it.skip("Update the sanitizer of the signed zone captain", async () => {
  ...
  it.skip("Revert: Try to deploy the Captain with an invalid deployer", async () => {
    const SignedZoneCaptainFactory = await ethers.getContractFactory(
      "SignedZoneCaptain",
      owner
    );
  ...
  it.skip("Revert: Try to deploy the Captain with a null address as initial owner.", async () => {
    const SignedZoneCaptainFactory = await ethers.getContractFactory(
      "SignedZoneCaptain",
      owner
    );
  ...
  it.skip("Add multiple signers, then remove them", async () => {
  ...
  it.skip("Only the owner or rotator can set and remove signers", async () => {
    await expect(
      signedZoneController
        .connect(buyer)
  ...
  it.skip("Sanitizer should be able to sanitize the zone.", async () => {
  ...
  it.skip("Rotator should be able to rotate signers.", async () => {
  ...
  it.skip("Create a zone setting initial owner not as caller.", async () => {
  ...
  it.skip("Revert: Try to create the zone with a previously used salt", async () => {
    const newSalt = `0x${owner.address.slice(2)}000000000000000000000099`;
  ...
  it.skip("Revert: Try to create the zone with an invalid creator", async () => {
  ...
  it.skip("Revert: Try to create the zone with an invalid initial owner", async () => {
    const newSalt =
  ...
  it.skip("Revert: Try to create a signed zone captain with an invalid controller", async () => {
    const SignedZoneCaptainFactory = await ethers.getContractFactory(
      "SignedZoneCaptain",
      owner
    );
  ...
  it.skip("Only the owner should be able to modify the apiEndpoint", async () => {
    expect(
      (
  ...
  it.skip("Only the owner should be able to modify the documentationURI", async () => {
    expect(
      (
        await signedZoneController.getAdditionalZoneInformation(
          signedZone.address
  ...
  it.skip("Should error on improperly formatted extraData", async () => {
    // Execute 721 <=> ETH order
    const nftId = await mintAndApprove721(seller, marketplaceContract.address);
