...
describe(`Basic buy now or accept offer flows (Seaport v${VERSION})`, function () {
  ...
  async function setupFixture() {
    ...
    return { seller, buyer, zone, sellerContract };
  }
  ...
  describe("A single ERC721 is to be transferred", async () => {
    describe("[Buy now] User fulfills a sell order for a single ERC721", async () => {
      ...
      it("ERC721 <=> ETH (basic, minimal and listed off-chain, efficient endpoint)", async () => {
        // Seller mints nft
        const nftId = await mintAndApprove721(
          seller,
          marketplaceContract.address
      ...
      it("ERC721 <=> ETH (basic, minimal and verified on-chain, efficient endpoint)", async () => {
        // Seller mints nft
        const nftId = await mintAndApprove721(
          seller,
          marketplaceContract.address
