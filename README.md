...
- As criteria resolvers are applied prior to item fulfillment, only a single criteria resolver can be applied to a specific item per order fulfillment. This means that criteria-based order items with amounts greater than one cannot be filled for multiple ERC721 or ERC1155 identifiers at once. One workaround is to fulfill orders via a batch of calls, though fulfillment aggregation cannot be applied when using this method. Furthermore, fulfilling these orders via a batch of calls implies that the fulfiller will need to either sign for each fulfillment or fulfill the orders through a contract that supports batch calls. A more robust solution would be to include an amount to resolve on criteria resolution and to generate new, non-criteria-based offer or consideration items (and decrement the criteria-based item being resolved) when only a portion of a given criteria-based item is being resolved to a specific identifier.
