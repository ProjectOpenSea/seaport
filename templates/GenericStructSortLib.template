...
library <T>SortLib {
    function key(<T> memory component) internal pure returns (uint256);

    function sort(<T>[] memory components) internal pure {
        sort(components, key);
    }
    ...
    function sort(
        <T>[] memory a,
        function(<T> memory) internal pure returns (uint256) accessor
    ) internal pure {
        if (a.length < 2) {
            return;
        }
